...
public class Splash extends Effect {
    ...
    {
        gs = new GameSprite("images/skills/sludge-bomb.png", 20, 20);
    }
    ...
    public Effect init(float x, float y, float rotation, float speedX, float speedY, float speedRotation){
        ...
        for(int i = 0; i < 10; i++){
            xyArrayList.add(new XY(0 , 0, speedX / 10 + 100f - RandomHelper.getInstance().randomFloat(100),100f + RandomHelper.getInstance().randomFloat(300)));
        }

        return this;
    }

    public Splash setTint(Color color) {
