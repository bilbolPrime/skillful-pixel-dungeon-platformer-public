...
public class FreezingCloud extends Unit {
    private static final float DURATION = 4f;
    private static final float EFFECT_INTERVAL = 0.8f;
    ...
    public FreezingCloud setOwner(Unit owner) {
        this.owner = owner;
        return this;
    }
    ...
    private void affectUnitsInside() {
        ...
        for (Unit unit : UnitHelper.getInstance().getUnitsSnapshot()) {
            if (unit == this || unit.showOnly() || unit.getRoom() == null || !unit.getRoom().equals(room) || unit.getHP() < 1 || unit.isDead()) {
                continue;
            }

            if (area.overlaps(unit.getHitBox())) {
                new Slow().setPermanent(false).setDuration(2.5f).setOwner(unit);
